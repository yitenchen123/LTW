...
void glTexBuffer(GLenum target, GLenum internalFormat, GLuint buffer) {
    if(!current_context) return;
    if(current_context->es32 && es3_functions.glTexBuffer) es3_functions.glTexBuffer(target, internalFormat, buffer);
    else if(current_context->buffer_texture_ext && es3_functions.glTexBufferEXT) es3_functions.glTexBufferEXT(target, internalFormat, buffer);
...
void glTexBufferRange(GLenum target, GLenum internalFormat, GLuint buffer, GLintptr offset, GLsizeiptr size) {
    if(!current_context) return;
    if(current_context->es32 && es3_functions.glTexBufferRange) es3_functions.glTexBufferRange(target, internalFormat, buffer, offset, size);
    else if(current_context->buffer_texture_ext && es3_functions.glTexBufferRangeEXT) es3_functions.glTexBufferRangeEXT(target, internalFormat, buffer, offset, size);
    else if(!buf_tex_trigger) {
        buf_tex_trigger = true;
        printf("LTW: Buffer textures aren't supported on your device\n");
    }
